...
if (spollersArray.length > 0) {
	...
	function initSpollers(spollersArray, matchMedia = false) {
		spollersArray.forEach(spollersColumn => {
			spollersColumn = matchMedia ? spollersColumn.item : spollersColumn;
			if (matchMedia.matches || !matchMedia) {
				spollersColumn.classList.add('_init');
				initSpollerBody(spollersColumn);
				spollersColumn.addEventListener("click", setSpollerAction);
			} else {
				spollersColumn.classList.remove('_init');
				initSpollerBody(spollersColumn, false);
				spollersColumn.removeEventListener("click", setSpollerAction);
			}
		});
	}
	// Работа с контентом
	function initSpollerBody(spollersColumn, hideSpollerBody = true) {
		const spollerTitles = spollersColumn.querySelectorAll('[data-spoller]');
	...
	function setSpollerAction(e) {
		...
		if (el.hasAttribute('data-spoller') || el.closest('[data-spoller]')) {
			...
			const spollersColumn = spollerTitle.closest('[data-spollers]');
			const oneSpoller = spollersColumn.hasAttribute('data-one-spoller') ? true : false;
			if (!spollersColumn.querySelectorAll('._slide').length) {
				if (oneSpoller && !spollerTitle.classList.contains('_active')) {
					hideSpollersBody(spollersColumn);
				}
	...
	function hideSpollersBody(spollersColumn) {
		const spollerActiveTitle = spollersColumn.querySelector('[data-spoller]._active');
